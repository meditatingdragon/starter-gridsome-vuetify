<template>
  <nav>
    <v-toolbar
      color="teal darken-1 overflow-hidden"
      :elevate-on-scroll="true"
      :hide-on-scroll="true"
      dark
    >
      <v-app-bar-nav-icon class="d-flex d-sm-none" @click="toggle"></v-app-bar-nav-icon>
      <v-btn text large href="/">
        <g-image src="~/favicon.png" height="35" width="35" class="mr-5" />
        <span class="headline font-weight-bold">{{$static.metadata.siteName}}</span>
      </v-btn>
      ...
      <v-navigation-drawer absolute v-model="collapse" :mini-variant.sync="mini" class="d-flex d-sm-none dark">
        <v-list-item>
          <v-list-item-avatar>
